...
import { Plus, Filter, DollarSign, ArrowUpCircle, ArrowDownCircle, Trash2, AlertCircle } from 'lucide-react';
...
const TransacoesPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (activeTab === 'despesas' && tipo === 'cartao' && (!cartaoId || parcelas < 1)) {
    ...
    try {
      ...
      if (activeTab === 'despesas' && tipo === 'cartao') {
        ...
        for (let i = 0; i < parcelas; i++) {
          ...
          try {
            ...
            fatura = faturas.find((f: any) => f.mes_referencia === mes_referencia && f.ano_referencia === ano_referencia);
          } catch (err: any) {
          ...
          if (!fatura) {
            ...
            if (faturaRes.status === 401) {
              setError('Sessão expirada. Faça login novamente.');
              setLoading(false);
              return;
            }
            if (!faturaRes.ok) {
              const err = await faturaRes.json();
              setError(err.error || 'Erro ao criar fatura');
              setLoading(false);
              return;
            }
            fatura = await faturaRes.json();
          }
          faturaId = fatura.id;
      ...
      setError(error.response?.data?.error || `Erro ao salvar ${activeTab === 'despesas' ? 'despesa' : 'receita'}`);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              >
                ...
                {(activeTab === 'despesas' ? categoriasDespesas : categoriasReceitas).map(cat => (
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-4">
                  <p className={`font-medium ${
                    activeTab === 'despesas' ? 'text-red-500' : 'text-green-500'
                  }`}>
                    {activeTab === 'despesas' ? '-' : '+'}{formatCurrency(item.valor)}
                  </p>
                  
                  <button
                    onClick={() => handleDelete(item.id, activeTab === 'despesas' ? 'despesa' : 'receita')}
